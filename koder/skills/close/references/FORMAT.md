...
Touched Repos `<clean paths>` or `BLOCKED — <path and reason>`
Commits       `<count>` this session across `<count>` repo(s)
...
- **Touched Repos** covers only repositories intentionally changed in this root session. Report preserved pre-existing dirt elsewhere in **Notes**, not as work completed.
